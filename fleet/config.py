...
def load_config() -> Config:
    """Read configuration from the environment. Raises on missing required values."""
    ssh_target = os.environ.get("DARKBLOOM_SSH_TARGET")
    database_url = os.environ.get("DATABASE_URL")
    if not ssh_target:
        raise RuntimeError("DARKBLOOM_SSH_TARGET is required (an SSH host or alias, e.g. from ~/.ssh/config)")
    if not database_url:
        raise RuntimeError("DATABASE_URL is required (postgres connection string)")

    models_raw = os.environ.get("DARKBLOOM_MODELS")
    models = tuple(m.strip() for m in models_raw.split(",") if m.strip()) if models_raw else DEFAULT_MODELS

    return Config(
        ssh_target=ssh_target,
        ssh_key_path=os.environ.get("DARKBLOOM_SSH_KEY_PATH") or None,
        remote_python=os.environ.get("DARKBLOOM_REMOTE_PYTHON", "python3"),
        host_label=os.environ.get("DARKBLOOM_HOST_LABEL", "host-1"),
        host_spec=os.environ.get("DARKBLOOM_HOST_SPEC", "unknown"),
        ...
        live_execution=_bool_env("FLEET_LIVE_EXECUTION", False),
